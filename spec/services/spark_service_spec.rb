...
describe SparkService, type: :service do
  ...
  context 'should be able to handle specific filters' do
    scenario 'for zip code' do
      expect(SparkService.build_filter({ 'zip' => 87114 })).to eq("PostalCode Eq '87114'")
    end

    scenario 'for min price' do
      expect(SparkService.build_filter({ 'minPrice' => 1500 })).to eq("ListPrice Ge 1500")
    end

    scenario 'for max price' do
      expect(SparkService.build_filter({ 'maxPrice' => 1500 })).to eq("ListPrice Le 1500")
    end

    scenario 'for min baths' do
      expect(SparkService.build_filter({ 'minBaths' => 1 })).to eq("BathsTotal Ge 1")
    end

    scenario 'for max baths' do
      expect(SparkService.build_filter({ 'maxBaths' => 3 })).to eq("BathsTotal Le 3")
    end

    scenario 'for min beds' do
      expect(SparkService.build_filter({ 'minBeds' => 3 })).to eq("BedsTotal Ge 3")
    end

    scenario 'for max beds' do
      expect(SparkService.build_filter({ 'maxBeds' => 3 })).to eq("BedsTotal Le 3")
    end

    scenario 'for min sqft' do
      expect(SparkService.build_filter({ 'minSqft' => 3 })).to eq("BuildingAreaTotal Ge 3")
    end

    scenario 'for max sqft' do
      expect(SparkService.build_filter({ 'maxSqft' => 5 })).to eq("BuildingAreaTotal Le 5")
    end
  end
end
